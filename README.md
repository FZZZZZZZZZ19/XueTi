...
一款简洁的**英语学习 App**。v1.6 版本包含：六级单词每日学习（完整词库 6662 词）、**六级学习合并页**、**全书学习（AI 生成教材目录 + 逐节三模块讲解）**、**AI 识图解题**、学习统计与日历、目标日倒计时，以及外观定制、应用内更新与每日提醒。

> 📦 **直接下载安装**：[GitHub Releases v1.6](https://github.com/FZZZZZZZZZ19/XueTi/releases/tag/v1.6)（`XueTi-v1.6-debug.apk`，Debug 签名，手机下载后直接安装）
...
| **AI 识图解题**（v1.3/v1.4） | 拍照 / 相册选图 / 纯文字三种出题方式；**图片可裁剪**（拖拽四角 + 旋转 90°）；**图像增强**（灰度化 / 自动对比度 / **高斯锐化** / Otsu 二值化）让题目文字更清晰；**用户自备 DeepSeek API Key 并自定义提示词**；默认 `deepseek-flash` 多模态模型，答案可一键复制 |
| **六级学习**（v1.6，合并页） | 把「今日学习进度 + 学习统计概览 + 我的词库」合并为一个页面：今日进度条与开始学习按钮、今日/连续/累计/认识率四项概览、已学单词列表（带认识/需复习标记） |
...
| **学习统计 + 日历**（v1.4/v1.5） | 今日已学 / 连续打卡 / 累计已学 / 认识率概览；近 7 天柱状图；**学习日历**（可翻月，标注每天学习数量、达标状态与**目标日**）；**点击日期可自定义该日学习量**（跟随默认 / 休息日 / 5·10·20·30·50 词）；今日学习内容逐词列表 |
...
`apk/XueTi-v1.6-debug.apk`（Debug 签名，直接安装到手机）

```bash
adb install -r apk/XueTi-v1.6-debug.apk
```
...
│   ├── StudyHubActivity.kt              # 六级学习（进度 + 统计概览 + 我的词库，合并页）
